...
Text::Text()
{
    this->textShader = new Shader("resources/shaders/text.vert", "resources/shaders/text.frag");

    glm::mat4 projection = glm::ortho(0.0f, static_cast<float>(WINDOW_WIDTH), 0.0f, static_cast<float>(WINDOW_HEIGHT));

    this->textShader->use();

    glUniformMatrix4fv(glGetUniformLocation(this->textShader->getID(), "projection"), 1, GL_FALSE, glm::value_ptr(projection));
    ...
    // find path to font
    std::string font_name = "resources/fonts/Inconsolata.ttf";

    if (font_name.empty())
        throw std::invalid_argument("ERROR::FREETYPE: Failed to find font_name.");
...
void Text::loadCharacters(FT_Face &face)
{
    ...
    for (unsigned char c = 0; c < 128; c++)
    {
        ...
        if (FT_Load_Char(face, c, FT_LOAD_RENDER))
            throw std::invalid_argument("ERROR::FREETYTPE: Failed to load Glyph");
