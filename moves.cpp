...
MoveAbstract &MoveAbstract::operator=(const std::unique_ptr<MoveAbstract> &moves)
{
...
void MoveAbstract::effect(const std::unique_ptr<std::unique_ptr<Pokemon>[]> &trainer, const std::unique_ptr<std::unique_ptr<Pokemon>[]> &opponent, const us userIndex, const BattleType type) const
{
    if (this->name == "Growl")
    {
        // Growl will affect the pokemon in front of it, and also to the left and right if possible
        if (userIndex > 0)
            opponent[static_cast<unsigned long>(userIndex - 1)]->getStats()->setDefenseMultiplier(0.5);
        if (userIndex < type)
            opponent[static_cast<unsigned long>(userIndex + 1)]->getStats()->setDefenseMultiplier(0.5);
